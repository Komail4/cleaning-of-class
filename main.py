...
delete_frame = tkinter.Frame(container)

for frame in (input_info, output_daily, output_weekly, output_monthly, output_total, delete_frame):
...
# label_class_delete = tkinter.Label(delete_frame, text=":صنف")
# label_class_delete.pack(pady=5)

# combo_class_delete = ttk.Combobox(delete_frame, values=class_numbers, state="readonly")
# combo_class_delete.pack(pady=5)
...
# button_submit_delete = tkinter.Button(delete_frame, text = "Submit", width=10, bg="green")
# button_submit_delete.pack(pady=5)


# Add widgets for selecting class and date in the delete_frame
label_class_delete = tkinter.Label(delete_frame, text=":صنف")
label_class_delete.pack(pady=5)
...
label_date_delete = tkinter.Label(delete_frame, text=":تاریخ")
label_date_delete.pack(pady=5)

combo_date_delete = ttk.Combobox(delete_frame, state="readonly")
combo_date_delete.pack(pady=5)

# Function to populate the date dropdown based on the selected class
def populate_dates():
    selected_class = combo_class_delete.get()
    ...
    if selected_class in data:
        available_dates = list(data[selected_class].keys())
        combo_date_delete["values"] = available_dates
        combo_date_delete.set("")  # Clear the current selection
    else:
        combo_date_delete["values"] = []
        messagebox.showinfo("Info", "هیچ اطلاعاتی برای این صنف موجود نیست")

# Bind the class selection to populate the dates
combo_class_delete.bind("<<ComboboxSelected>>", lambda e: populate_dates())

# Function to delete the selected date for the selected class
def delete():
    selected_class = combo_class_delete.get()
    selected_date = combo_date_delete.get()

    if not (selected_class and selected_date):
        messagebox.showwarning("Warning", "لطفا صنف و تاریخ را انتخاب کنید")
        return
    ...
    if selected_class in data and selected_date in data[selected_class]:
        # Confirm deletion
        confirm = messagebox.askyesno("Confirm Deletion", f"آیا میخواهید اطلاعات تاریخ {selected_date} را حذف کنید؟")
        if confirm:
            del data[selected_class][selected_date]
            # Remove the class if no dates are left
            ...
            messagebox.showinfo("Success", "اطلاعات با موفقیت حذف شد")
            populate_dates()  # Refresh the date list
        else:
            messagebox.showinfo("Cancelled", "حذف اطلاعات لغو شد")
    else:
        messagebox.showwarning("Warning", "اطلاعات پیدا نشد")

# Add the delete button
button_submit_delete = tkinter.Button(delete_frame, text="حذف", width=10, bg="red", command=delete)
button_submit_delete.pack(pady=5)
...
delete_menu = tkinter.Menu(menu_bar, tearoff=0)
delete_menu.add_command(label="Delete", command=lambda:show_frame(delete_frame))
